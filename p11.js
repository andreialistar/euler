...
for(var i =0; i< grid.length; i++)
    for(var j=0; j< grid[i].length-3; j++){
        ...
        if (p > verticalProd){ verticalProd = p;
         console.log(grid[i][j]+" "+grid[i][j+1]+" "+grid[i][j+2]+" "+grid[i][j+3]);
                             }
    }
...
for(var i =0; i< grid.length-3; i++)
    for(var j=0; j< grid[i].length; j++){
        var p = grid[i][j]*grid[i+1][j]*grid[i+2][j] * grid[i+3][j];
               if (p > horiProd){
                   horiProd = p;
                   console.log( grid[i][j]+" "+grid[i+1][j]+" "+grid[i+2][j] +" "+ grid[i+3][j]);
               }
    }
console.log("\n");
var diagProd = 1;
for(var i =0; i< grid.length-3; i++)
    for(var j=0; j< grid[i].length-3; j++){
        var p = grid[j][i]*grid[j+1][i+1]*grid[j+2][i+2] * grid[j+3][i+3];
...
console.log(horiProd);
console.log(diagProd);